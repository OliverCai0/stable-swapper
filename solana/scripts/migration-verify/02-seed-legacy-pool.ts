...
async function main() {
  ...
  const idlPath = path.join(ARTIFACTS_LEGACY_DIR, "scaas_liquidity.json");
  ...
  const userAtaA = await getOrCreateAta(connection, payer, mintA, payer.publicKey);
  const userAtaB = await getOrCreateAta(connection, payer, mintB, payer.publicKey);

  const units = (n: number) => BigInt(n) * BigInt(10 ** DECIMALS);
  await mintTo(connection, payer, mintA, userAtaA, payer, units(USER_MINT_TOKENS));
  await mintTo(connection, payer, mintB, userAtaB, payer, units(USER_MINT_TOKENS));
  ...
  const minOut = new anchor.BN((SWAP_TOKENS * 10 ** DECIMALS * 0.99).toString());
