...
cp "$DEPLOY_KEYPAIR" "$LEGACY_SOLANA/target/deploy/scaas_liquidity-keypair.json"

# Align program ID for the cluster section we care about (+ sibling sections if present).
patch_program_id \
  "$LEGACY_SOLANA/programs/scaas-liquidity/src/lib.rs" \
...
cp "$LEGACY_SOLANA/target/deploy/scaas_liquidity.so" \
  "$ARTIFACTS_LEGACY/scaas_liquidity.so"
cp "$LEGACY_SOLANA/target/idl/scaas_liquidity.json" \
  "$ARTIFACTS_LEGACY/scaas_liquidity.json"
