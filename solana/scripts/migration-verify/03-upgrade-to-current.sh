...
cp "$SOLANA_DIR/target/deploy/stable_swapper.so" \
  "$ARTIFACTS_CURRENT/stable_swapper.so"
cp "$SOLANA_DIR/target/idl/stable_swapper.json" \
  "$ARTIFACTS_CURRENT/stable_swapper.json"
...
(
  cd "$SOLANA_DIR"
  anchor idl upgrade --filepath target/idl/stable_swapper.json "$PROGRAM_ID" \
    --provider.cluster "$MIGRATION_VERIFY_CLUSTER" 2>/dev/null || \
  anchor idl init --filepath target/idl/stable_swapper.json "$PROGRAM_ID" \
    --provider.cluster "$MIGRATION_VERIFY_CLUSTER" 2>/dev/null || \
